...
def save_image_individual(img, lms, fname, drange):
    lo, hi = drange
    img = np.asarray(img, dtype=np.float32)
    img = (img - lo) * (255 / (hi - lo))
    img = np.rint(img).clip(0, 255).astype(np.uint8)
    ...
    assert C in [1, 3]
    for i, im_lm in enumerate(zip(img,lms)):
        if C == 1:
            PIL.Image.fromarray(im[:, :, 0], 'L').save(fname)
        if C == 3:
            PIL.Image.fromarray(im_lm[0], 'RGB').save(fname+f'_{i}.png')
            f = open(f'./{fname}_{i}.txt', 'a')
            np.savetxt(f, im_lm[1], fmt='%f')
            f.close()
...
def training_loop(
    ...
    with dnnlib.util.open_url(resume_pkl) as f:
        ...
        G = resume_data['G'].to(device).train()
        D = resume_data['D'].to(device).train()
    ...
    if rank == 0:
        ...
        grid_z = torch.from_numpy(np.random.RandomState(0).randn(labels.shape[0], G.z_dim)).to(device).split(batch_gpu)
        grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)
        # images_ema = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
        # images = torch.cat([G(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
        # save_image_grid(images_ema, os.path.join(run_dir, 'fakes_ema_init.png'), drange=[-1, 1], grid_size=grid_size)
        ...
        with torch.no_grad():
            ...
            for z, c in zip(grid_z, grid_c):
                composed_image, tex_image_orig, ws = loss.run_G_3D(z, c, True)
                img_dict.append(composed_image.cpu().numpy())
                tex_dict.append(tex_image_orig.cpu().numpy())
                # lms_dict.append(lms)
            composed_images = np.concatenate(img_dict)
            texture_images = np.concatenate(tex_dict)
            # lms = np.concatenate(lms_dict, axis=0)

            save_image_grid(composed_images, os.path.join(run_dir, f'fakes_init.png'), drange=[-1, 1],
                            grid_size=grid_size)
            # save_image_individual(texture_images, lms, os.path.join(run_dir, f'fakes_init'), drange=[-1, 1])
            save_image_grid(texture_images, os.path.join(run_dir, f'tex_init.png'), drange=[-1, 1],
                            grid_size=grid_size)
    ...
    while True:
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images_ema = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
            # images = torch.cat([G(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
            save_image_grid(images_ema, os.path.join(run_dir, f'fakes_ema{cur_nimg // 1000:06d}.png'), drange=[-1, 1],
                            grid_size=grid_size)
            ...
            save_image_grid(texture_images, os.path.join(run_dir, f'fakes_tex{cur_nimg // 1000:06d}.png'),
                            drange=[-1, 1], grid_size=grid_size)
            save_image_grid(composed_images, os.path.join(run_dir, f'fakes{cur_nimg // 1000:06d}.png'), drange=[-1, 1],
                            grid_size=grid_size)
