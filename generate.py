...
def generate_images(
    ...
    with dnnlib.util.open_url(network_pkl) as f:
        G = legacy.load_network_pkl(f)['G'].to(device).eval() # type: ignore
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        for i in range(num_images):
            ...
            ws = G.mapping(z.to(device), label, truncation_psi=truncation_psi)
            cutoff = torch.empty([], dtype=torch.int64, device=ws.device).random_(1, ws.shape[1])
            cutoff = torch.where(torch.rand([], device=ws.device) < style_mixing_prob, cutoff,
                                 torch.full_like(cutoff, ws.shape[1]))
            f_name = f'{outdir}/{i:06d}.npz'
            if os.path.exists(f_name):
                print(f'{f_name} file exists. Skipping.')
                torch.randn_like(z)
                continue
            ws[:, cutoff:] = G.mapping(torch.randn_like(z), label, truncation_psi=truncation_psi)[:, cutoff:]
            img = G.synthesis(ws, noise_mode=noise_mode)
            img = (img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
            PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/{i:06d}.png')
